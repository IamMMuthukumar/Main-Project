...
while 1:
	...
	for face_id, tracker in trackers.items(): # iterate over current trackers
		...
		cursor = conn.cursor()
		cursor.execute("SELECT  *  FROM regtb where  Regno='" + str(ss) + "'")
		...
		if data:
			...
			if data is None:
				...
				conn.close()
				print(stnumber)
				sendmsg(stnumber, "Present For College Today")
		...
		'''conn = mysql.connector.connect(user='root', password='', host='localhost', database='1faceexamdb')
        cursor = conn.cursor()
        cursor.execute("select * from regtb where Degree='" + str(Degree1) + "' and Department	='" + str(Department1) + "' and Year='"+ str(Year1) +"' and Regno='"+str(ss) +"'")
        data = cursor.fetchone()
        if data is None:
            print("No Exam Date Found")
            #conn = mysql.connector.connect(user='root', password='', host='localhost', database='staff')
            #cursor = conn.cursor()
            #cursor.execute(

                #"insert into staffatten values('','" + str(ss) + "','" + str(ss) + "','" + str(
                    #date) + "','" + str(timeStamp) + "','')")
            #conn.commit()
            #conn.close()
		...
		# conn = mysql.connector.connect(user='root', password='', host='localhost', database='1faceexamdb')
		# cursor = conn.cursor()
		# cursor.execute("update staffatten set otime='" + str(timeStamp) + "'")
		# conn.commit()
		# conn.close()
